    ...
    for (int i = 0; i < DIR_ENTRIES_PER_BLOCK; i++) {
        ...
        if(buffer[0] == 3) { // archivo:
            ...
            if (strcmp(path2, filename) == 0) { // compara con filename
                ...
                printf("Puntero indice: %d\n", puntero_indice);
                printf("Puntero indice: %ld\n", buffer[1]);
                // for (int j = 1; j <= 5; j++) {
                //     int bit = (buffer[i] & (1 << j)) >> j; // Shift left para sacar el bit
                //     printf("%d", bit );
                //     bit ? fill++ : free++; // Se ve más cool así
                // }
                printf("Byte del indice 1: %x\n", buffer[1]);
                printf("Byte del indice 2: %x\n", buffer[2]);
                printf("Byte del indice 3: %x\n", buffer[3]);
                printf("Byte del indice 4: %x\n", buffer[4]);
                printf("Byte del indice 5: %x\n", buffer[5]);
                printf("Byte del indice 6: %x\n", buffer[6]);
                printf("Byte del indice 7: %x\n", buffer[7]);
                printf("Byte del indice 8: %x\n", buffer[8]);
    ...
    for (int i = 0; i < DIR_ENTRIES_PER_BLOCK; i++) {
        ...
        if(buffer[0] == 3){ // archivo:
            ...
            if (strcmp(path, filename) == 0) { // compara con filename
                fclose(f); // Evitamos leaks
                long int puntero_indice = buffer[1]; // Pesco los bytes 1-4 en adelante
                printf("Puntero indice: %d\n", puntero_indice);
                printf("Puntero indice: %lld\n", buffer[1]);
                printf("Byte del indice 1: %x\n", buffer[1]);
                printf("Byte del indice 2: %x\n", buffer[2]);
                printf("Byte del indice 3: %x\n", buffer[3]);
                printf("Byte del indice 4: %x\n", buffer[4]);
